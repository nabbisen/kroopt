...
def extSigAlgsOther : List UInt8 := [0, 0x0d, 0, 4, 0, 2, 0x08, 0x04]  -- rsa_pss_rsae_sha256 only (not presentable)
...
def checks : List Check :=
  ...
  , { name := "ClientHello offering only RSA-PSS signature_algorithms is refused (not presentable, RFC 033)"
