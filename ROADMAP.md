...
**Status (M20, package 0.21.0-dev) — Ed25519 root-cause isolation + SHA-512 KAT hardening.**
Done: isolated the M19 Ed25519 interop defect to its true source and hardened the crypto KATs. Bound `Hacl.sha512` and added a SHA-512 FIPS 180-4 value KAT; upgraded SHA-384 from size-only to a value KAT (`kroopt-hacl-test` now 15 checks). 17 suites + fuzz + 3 gates green; **87** theorems unchanged (Crypto/Native zone).

**⚠ Top pending — re-vendor a correct Ed25519 (interop blocker).** M20 corrected the M19 guess: the vendored `Hacl_Ed25519.c` and all its dependencies are **byte-identical to pristine HACL 0.4.5** (`ocaml-v0.4.5`, `diff` = 0) — *not* hand-edited (the odd `sign_expanded` parameter naming is original 0.4.5 codegen). A standalone C KAT on pristine 0.4.5 sources (no kroopt FFI) reproduces the wrong public `bcd55c06…` (vs RFC 8032 `d75a9801…`) identically at `-O0/-O1/-O2/-O2 -fno-strict-aliasing`, so the fault is **not** the FFI, optimisation, or strict aliasing — it is HACL 0.4.5 `dist/gcc-compatible` Ed25519's Edwards arithmetic as built here. SHA-256/384/512 (FIPS 180-4) and X25519 (RFC 7748) are confirmed correct. Remediation: re-vendor a known-correct Ed25519 unit (a newer HACL release), **KAT-validated standalone against RFC 8032 before integration**, as a quarantined unit with recorded provenance; the `kroopt-provision-test` tripwire guards the seam and flips when fixed. After it: the structural-to-real handshake work — resolve the schedule's abstract transcript snapshot references to real transcript hashes, compute the real server-Finished MAC, then a real handshake against OpenSSL/curl. Still pending: P-256 (HACL `Hacl_P256.c` is not vendored — header only), ASan/UBSan jobs, the iotakt `Transport` adapter, microbenchmarks.
