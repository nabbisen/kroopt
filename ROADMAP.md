...
**Status (M16, package 0.17.0-dev) — orchestrator interleaved into two stages.**
Done: corrected the orchestrator's derivation timing to TLS 1.3 — `start` runs the handshake-key stage (over the CH..ServerHello transcript) and parks at a `handshakeKeysInstalled` pause; `resumeApplication` supplies the CH..server-Finished transcript and runs the application-key stage to `complete`. Proofs extended to cover both stages — `advance_progress` excludes the pause, plus `advance_pause_inert`, `resumeApplication_emits_schedule_ops`, `resumeApplication_progress` (→ **85** theorems). `kroopt-scheduledriver-test` (12 checks) drives both stages through the real provider and validates each against RFC 8448 §3. The orchestrator now matches the handshake's interleaving but is still **not invoked by `step`**. Pending and scoped next (M17): the now-faithful wiring is two insertions — pump the handshake-key stage after ServerHello is framed, then `resumeApplication` and pump the application-key stage after the server Finished is committed — re-establishing the absence/legality proofs (expected to hold). Then production entropy + certificate provisioning, then a real handshake against OpenSSL/curl. Still pending: P-256, ASan/UBSan jobs, the iotakt `Transport` adapter, microbenchmarks.
