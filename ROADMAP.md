...
**Status (M19, package 0.20.0-dev) — connection provisioning + crypto KAT hardening; Ed25519 defect found.**
Done: `Kroopt.Crypto.Provision` adds production connection provisioning — `genEphemeralX25519` draws a fresh ephemeral X25519 key pair from the OS CSPRNG (`Hacl.randomBytes`) per connection (no longer injected), and `CertProvision` + `Provision.lint` / `lintAgainstClaimed` derive the certificate leaf public from a signing seed and fail closed at load on a bad length, unsupported scheme, or mis-paired claimed key; `provisionRealConfig` assembles a `RealCryptoConfig` from linted material plus a fresh ephemeral pair. New `kroopt-provision-test` (16 checks) covers ephemeral liveness/determinism, the lint branches, and the certificate-key sign+verify round-trip, and adds a SHA-384 FIPS 180-4 value-KAT (the HACL suite previously only size-checked it). 17 suites + fuzz + 3 gates green; theorem count unchanged at **87** (provisioning is `Crypto`-zone, no proof obligations).

**⚠ Top pending — fix the vendored Ed25519 (interop blocker).** The KAT hardening surfaced that the vendored HACL **Ed25519 is not RFC 8032 compliant**: `sign(seed, "")` = `6b66cdc2…` not the RFC 8032 §7.1 signature `e5564300…`, and `secret_to_public` = `bcd55c06…` not `d75a9801…`. It is self-consistent (so round-trip tests passed and hid the defect) but a real peer would reject a `CertificateVerify` signed this way. SHA-384/512, Curve25519, and the FFI shim argument order are all confirmed correct, localising the fault to `Hacl_Ed25519.c` (likely a vendored-version mismatch). Tracked by a test tripwire that flips when fixed. This must be resolved — by re-vendoring the matching `Hacl_Ed25519.c` (and recording its provenance) — before any real interop. After it: the structural-to-real handshake work — resolve the schedule's abstract transcript snapshot references to real transcript hashes, compute the real server-Finished MAC, then a real handshake against OpenSSL/curl. Still pending: P-256 (HACL `Hacl_P256.c` is not vendored — header only), ASan/UBSan jobs, the iotakt `Transport` adapter, microbenchmarks.
