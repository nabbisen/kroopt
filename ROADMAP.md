...
**Status (M17, package 0.18.0-dev) — handshake-key schedule stage wired into `step`.**
Done: the verified orchestrator is now invoked by `Kroopt.Core.step`. `State` gained a `keySched : Option KeyScheduleDriver.State` field; `onEcdheDone` frames ServerHello, installs the handshake epoch, and starts the handshake-key stage (→ `derivedHandshakeSecrets`); the new `onHsScheduleResult` pumps it — feeding each HKDF/install result to `advance`, emitting the next op, self-looping until the `handshakeKeysInstalled` pause, then framing EncryptedExtensions/Certificate and requesting the CertificateVerify signature. `handleCryptoResultCorrelated` now routes `hkdfSecret`/`keysInstalled` to the gating dispatch (previously dropped). Proofs: new `onHsScheduleResult_legal` plus re-established `onEcdheDone` legal/no-emit/no-accept and extended dispatch no-emit/no-accept (the pump emits only `callCrypto`/`writeTransport`) — global action-discipline and `connected_requires_finished_verified` unchanged (→ **86** theorems). The full synthetic handshake drives the stage end-to-end through `step` (`kroopt-e2e-test`, `kroopt-handshake-test`); all 16 suites + fuzz + 3 gates green. Pending and scoped next (M18): drive the **application-key stage** via `resumeApplication` (a second pump phase, after the server Finished is committed; application keys currently still use a placeholder). Then resolve the schedule's abstract transcript snapshot references to real transcript hashes, then production entropy + certificate provisioning, then a real handshake against OpenSSL/curl. Still pending: P-256, ASan/UBSan jobs, the iotakt `Transport` adapter, microbenchmarks.
