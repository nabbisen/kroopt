...
**Status (M15, package 0.16.0-dev) — verified key-schedule orchestrator delivered.**
Done: the schedule *sequence* is now a pure, proved core state machine (`Kroopt.Core.KeyScheduleDriver`) — fifteen phases from the ECDHE share to the installed application keys, emitting the schedule's ops and threading the secret handles. Four new theorems (→ **82** total): emits only ECDHE/HKDF/install ops (never plaintext/AEAD-open), monotone progress (finite, no loop), `complete` absorbing. Driven through `mkRealProvider` and validated against the RFC 8448 §3 trace (`kroopt-scheduledriver-test`, 11 checks). The orchestrator is **not yet invoked by `step`**, so the existing handshake proofs are untouched. Pending and scoped next (M16): wire the orchestrator into the live handshake — `onEcdheDone`/gating dispatch kick off and pump the schedule, threading its state through negotiation — re-establishing the absence/legality proofs (expected to hold: they are absence-dominated and the orchestrator emits only schedule ops). Then production entropy + certificate provisioning through the interpreter, then a real TLS 1.3 handshake on `TLS_CHACHA20_POLY1305_SHA256` against OpenSSL/curl. Still pending: P-256, ASan/UBSan jobs, the iotakt `Transport` adapter, microbenchmarks.
