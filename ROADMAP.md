...
**Status (M14, package 0.15.0-dev) — enriched seam + real provider delivered.**
Done: enriched `CryptoOp`/`CryptoResult` naming secret inputs by handle (HKDF salt/IKM/secret handles, `installTrafficKeys`, ECDHE `ecdheComplete`); AEAD shapes left fixed so **all 78 theorems held unchanged** (the proof migration was empty); a real `CryptoProvider` (`Kroopt.Crypto.mkRealProvider`) doing genuine HACL\* crypto through `submit`, **validated against the RFC 8448 §3 trace operation-by-operation** (`kroopt-realprovider-test`, 17 checks). Pending and scoped next: make `Kroopt.Core.step` *emit* the schedule sequence (thread handles through negotiation state — the interface and proofs are now ready, AEAD shapes fixed); seed production entropy and load the certificate key through the interpreter; then a real TLS 1.3 handshake on `TLS_CHACHA20_POLY1305_SHA256` (real wire messages + transcript) against OpenSSL/curl. Still pending: P-256, ASan/UBSan jobs, the iotakt `Transport` adapter, microbenchmarks.
