...
## Status: M0–M35 + M36-prelude (verified core → handshake → TlsConn → config → alerts/close → HTTPS → hardening → native crypto → key schedule → real provider → orchestrator → both schedule stages wired into step → connection provisioning → Ed25519 RFC 8032 KAT verified + OpenSSL CertificateVerify interop + crypto KAT hardening → RFC lifecycle migration: 19 implemented RFCs moved to rfcs/done/ → real handshake wire serializer + full server-flight framing + server-Finished MAC KAT vs RFC 8448 §3 → real server-flight assembler + Ed25519 CertificateVerify → live step-driven real handshake to **connected** → real ChaCha20-Poly1305 record protection → encrypted flight on the wire → real Ed25519 X.509 cert presentation → record-layer interop with Python cryptography → records over a real OS socket → M36-prelude (RFC 034): real provider capability honesty + fail-closed entropy → **M36 part 1 (RFC 033): the protected client Finished opens in the core**)
