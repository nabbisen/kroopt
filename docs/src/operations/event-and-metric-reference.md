...
| `alert-sent` | a fatal alert **record** was actually framed/queued for the wire (RFC 041; emitted from `execAction` only when a record is formed — never on the no-key/no-suite path) | connection id, alert description |
...
| `kroopt_alerts_sent_total` | wired (internal counter) | `alert` | fatal alert **records framed/queued for transport** (RFC 041; counted only when a record is actually formed, not on the no-key/no-suite path) — framing, **not** guaranteed peer delivery; `≤ kroopt_alerts_classified_total` |
