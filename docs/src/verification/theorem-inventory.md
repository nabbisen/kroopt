...
| 11c | `parseExtensionItems_input`, `parseServerNameItems_input`, `frameClientHelloBody_input`, `parseClientHelloBody_input`, `parseClientHello_exact_input` | Concrete extension/SNI/body callbacks preserve their input, and every successful public ClientHello parse binds the original bytes and proves `input.size = 4 + declared` for the decoded uint24 header length. | RFC 046 §§3.2, 5, 7, 9 | propext, Quot.sound | proved |
